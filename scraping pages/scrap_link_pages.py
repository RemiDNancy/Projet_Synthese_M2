...
JOURS_RESTANTS_MIN = 29  # nombre de jours restants minimum
TEMPS_PAUSE_MIN = 4
TEMPS_PAUSE_MAX = 5
    ...
    while len(liens) < NOMBRE_PROJETS:
        ...
        for card in cards:
            ...
            try:
                ...
                deadline = donnees_json.get("deadline")
                if deadline:
                    date_limite = datetime.fromtimestamp(deadline)
                    date_actuelle = datetime.now()
                    jours_restants = (date_limite - date_actuelle).days
                else:
                    jours_restants = 0
                ...
                # Vérifier que le projet a au moins 29 jours restants
                if jours_restants < JOURS_RESTANTS_MIN:
                    continue
                ...
                if mode_newest:
                    if pourcentage_finance <= SEUIL_FINANCEMENT and nb_faibles_recoltes < NOMBRE_PROJETS_FAIBLE:
                        ...
                        liens.append(lien)
                        print(f"    FAIBLE [{categorie}] {nom} - {pourcentage_finance:.1f}%")
                # En mode normal on collecte les deux types
                    ...
                    if pourcentage_finance <= SEUIL_FINANCEMENT and nb_faibles_recoltes < NOMBRE_PROJETS_FAIBLE:
                        ...
                        liens.append(lien)
                        print(f"    NORMAL [{categorie}] {nom} - {pourcentage_finance:.1f}%")
